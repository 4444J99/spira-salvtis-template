...
**Produces:** In-situ atom markers in `docs/archive/extracted/**/*.md`, computed `docs/archive/atom-registry.yaml`, coverage maps, phase strike plans
**SOP generated:** SOP-SS-ATM-001 (Atomic Decomposition & Coverage Proof)
**Architecture:** Atoms are marked in-place inside extracted files (HTML comment fences). The registry is a computed artifact generated by parsing these markers — not a separate source of truth.
...
- Provenance is ambiguous (is this admin's insight or ChatGPT's interpolation?)
...
### In-Situ Format (primary — lives inside extracted files)

Atoms are fenced with HTML comments directly in the extracted Markdown. The content between fences is untouched client material. Markdown renderers ignore the comments — files read clean.

```markdown
<!-- ATM-H-003 IN
idea: "90-second cortisol reset: box breathing through the activation window allows the cortisol wave to complete without reinforcement"
section: "Slide Content — The Reset"
position: 3
of_total: 7
prev: ATM-H-002
...
Cortisol operates on a 90-second metabolic cycle. If you can hold through
90 seconds of activation without reinforcing the stress response, the cortisol
wave completes and your nervous system returns to baseline. Box breathing
during this window is the intervention.
...
### Document Envelope (lives at top of each extracted file, above YAML frontmatter)
...
sections:
  - name: "Cortisol Science"
    atoms: [ATM-H-001, ATM-H-002, ATM-H-003]
  - name: "IG Carousel Slides"
    atoms: [ATM-H-004, ATM-H-005]
  - name: "Caption Drafts"
    atoms: [ATM-H-006, ATM-H-007]
-->
```

### Computed Registry (derived — generated by parsing in-situ markers)

A script walks `docs/archive/extracted/**/*.md`, parses all `ATM-*` comment fences, and emits `docs/archive/atom-registry.yaml`:

```yaml
# AUTO-GENERATED — do not edit. Source of truth is the in-situ markers.
# Regenerate: scripts/build-atom-registry.sh

atoms:
  - id: ATM-H-003
    source_file: health/cortisol-stress-carousel.md
    idea: "90-second cortisol reset: box breathing through activation window"
    section: "Slide Content — The Reset"
    position: 3
    of_total: 7
    prev: ATM-H-002
...
Coverage maps and strike plans are likewise computed views, not hand-maintained.
...
|-------|-----------|-----------------|---------|
| `LOCAL` | admin's original insight, experience, or decision — not generated by AI | Highest — client's authentic voice | Personal healing story, pricing decisions, "Feel Good First" philosophy origin |
| `HYBRID` | admin-directed AI output — she prompted, AI structured/expanded | Medium — AI-shaped but client-seeded | ChatGPT-generated protocols from her prompts, Gemini research summaries she requested |
| `ALIEN` | Pure AI generation with no clear admin input visible | Lowest — AI interpolation | Generic health facts, boilerplate definitions, AI-generated frameworks without personal direction |

### Provenance determination method

1. Read the file's prompt section (if ChatGPT/Gemini export — prompt is usually visible)
2. If admin's voice is in the prompt → atoms in the response are `HYBRID`
...
Cross-cutting view — where is admin's authentic voice vs AI expansion?

```
LOCAL: ~120 atoms — admin's own insights, stories, decisions
  → These are the HIGHEST PRIORITY for build injection
  → They cannot be recreated or approximated
  → They ARE the brand

HYBRID: ~950 atoms — admin-directed AI output
...
|----------|--------|----------|
| In-situ atom markers | HTML comments in Markdown | `docs/archive/extracted/**/*.md` |
| Document envelopes | HTML comment at file top | Same files, above frontmatter |

### Computed (derived — regenerable from primary)

| Artifact | Format | Location | Generator |
|----------|--------|----------|-----------|
| Atom registry | YAML | `docs/archive/atom-registry.yaml` | `scripts/build-atom-registry.sh` |
| Coverage map: by node | Markdown | `docs/archive/coverage/by-node.md` | Same script |
| Coverage map: by pillar | Markdown | `docs/archive/coverage/by-pillar.md` | Same script |
| Coverage map: by provenance | Markdown | `docs/archive/coverage/by-provenance.md` | Same script |
| Coverage map: by nature | Markdown | `docs/archive/coverage/by-nature.md` | Same script |
| Coverage proof | Markdown | `docs/archive/coverage/proof.md` | Same script |
| Phase strike plans | Markdown | `docs/archive/strikes/phase-{N}.md` | Same script |
...
| `handoff-admin-spiral-path-2026-04-01.md` | File inventory + readiness ratings → atom registry inherits and deepens |
