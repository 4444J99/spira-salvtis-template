/**
 * Spira salvtis Spiral — Three.js 3D Helix Renderer
 *
...
// Extension ABOVE the top (activate) node. Short by default so the authenticate/
// activate star sits at the very top (admin 2026-06-25), while the bottom keeps
...
function primaryLensForNode(nodeIndex: number, phase: string): Lens {
  ...
  if (phase === 'ACTIVATE') return idx % 3 === 0 ? baseLens : 'greek-classical';
  return baseLens;
}
...
export function initSpiral(
  ...
  try {
    ...
    PHASE_HEX.INITIATE = pHex('colors.initiate', PHASE_HEX.INITIATE);
    PHASE_HEX.INTEGRATE = pHex('colors.integrate', PHASE_HEX.INTEGRATE);
    PHASE_HEX.ACTIVATE = pHex('colors.activate', PHASE_HEX.ACTIVATE);
    ...
    const phaseC = {
      INITIATE: new THREE.Color(PHASE_HEX.INITIATE),
      INTEGRATE: new THREE.Color(PHASE_HEX.INTEGRATE),
      ACTIVATE: new THREE.Color(PHASE_HEX.ACTIVATE),
    };
    const totalRange = 1 + PATH_EXTEND + PATH_EXTEND_TOP;
    for (let i = 0; i < path.length; i++) {
    ...
    nodes.forEach((node, i) => {
      ...
      const pm = PHASE_MAT[phase] || PHASE_MAT.INITIATE;
      const pa = PHASE_ANIM[phase] || PHASE_ANIM.INITIATE;
