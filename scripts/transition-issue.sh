...
get_current_status() {
  local issue_num="$1"
  if [[ ${CURRENT_STATUS_KNOWN-} == "1" ]]; then
    echo "${CURRENT_STATUS-}"
    return 0
  fi
...
" 2>/dev/null)
    ...
    if [[ ${ALLOWED} == "__MISSING__" ]]; then
      echo "ERROR: No transitions configured for status '${CURRENT}'." >&2
      log_event "${ISSUE_NUM}" "REJECTED" "${CURRENT} → ${TARGET_STATUS} (unconfigured)" "${REASON}"
      exit 1
    fi
  ...
  if [[ ${TARGET_STATUS} == "CLOSED" ]]; then
    gh issue close "${ISSUE_NUM}" --repo "${REPO_SLUG}" --reason completed >/dev/null 2>&1 || true
...
field)
  ...
  *)
  ...
  case "${CONFIG_KEY}" in
  phase) FIELD_ID="${PHASE_FIELD}" ;;
  issue_type) FIELD_ID="${ISSUE_TYPE_FIELD}" ;;
  priority) FIELD_ID="${PRIORITY_FIELD}" ;;
  next_action) FIELD_ID="${NEXT_ACTION_FIELD}" ;;
  external_party) FIELD_ID="${EXTERNAL_PARTY_FIELD}" ;;
  *)
    echo "ERROR: No field id mapping for '${CONFIG_KEY}'." >&2
    exit 1
    ;;
  esac
